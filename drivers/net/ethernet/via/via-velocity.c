...
static int velocity_suspend(struct device *dev, pm_message_t state)
{
	struct net_device *netdev = dev_get_drvdata(dev);
	struct velocity_info *vptr = netdev_priv(netdev);
	unsigned long flags;
	...
	if (vptr->flags & VELOCITY_FLAGS_WOL_ENABLED) {
		...
		velocity_set_power_state(vptr,
				velocity_choose_state(vptr->dev, state));
	}
...
{
	return velocity_suspend(&pdev->dev, state);
}

static int velocity_pci_resume(struct pci_dev *pdev)
{
	return velocity_resume(&pdev->dev);
}
